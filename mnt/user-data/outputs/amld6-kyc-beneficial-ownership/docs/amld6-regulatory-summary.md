...
Six worked examples demonstrating end-to-end decision flows are available in [`test-scenarios.md`](test-scenarios.md). These cover LOW/MEDIUM/HIGH risk paths, delegated act thresholds, senior management fallback, analyst escalation, and correspondent banking — providing concrete evidence that the DMN logic produces correct outcomes.

### LLM-to-DMN Extraction

A proof of concept for semi-automated regulatory extraction is documented in [`llm-to-dmn-extraction-poc.md`](llm-to-dmn-extraction-poc.md). This demonstrates how an LLM can pre-extract decision rules from regulatory text (using AMLR Article 19 as a worked example), with a DMN Modeller validating the output — identifying a ~63% production-readiness rate for raw LLM extraction.
